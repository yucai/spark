...
private[spark] class DiskBlockManager(blockManager: BlockManager, conf: SparkConf)
  ...
  // org.apache.spark.network.shuffle.ExternalShuffleBlockResolver#hashAllocator().
  ...
  // This method should be kept in sync with
  // org.apache.spark.network.shuffle.ExternalShuffleBlockResolver#hierarchyAllocator().
  ...
  private[spark] class HierarchyAllocator extends FileAllocationStrategy {
    ...
    val allLayers: Array[LayerInfo] = hsLayers :+
      LayerInfo("Last Storage", (1 << 30).toLong, lastLayerDirs)
